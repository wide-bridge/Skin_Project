...
def _select_rows(rows: list[dict[str, Any]], terms: list[str], top_k: int) -> list[dict[str, Any]]:
...
def retrieve_dermatology_contexts(
    ...
    selected = _select_rows(corpus, primary_terms, top_k)
    if not selected and differentials:
        ...
        selected = _select_rows(corpus, differential_terms, top_k)
    if not selected:
